...
/// @author  Laura Bieker
/// @date    Sept 2002
/// @version $Id: GUIEdge.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
GUIEdge::GUIEdge
 ...
 :MSEdge(id, numericalID, streetName), GUIGlObject(GLO_EDGE, id)
{}

GUIEdge::~GUIEdge
 ()
{
  for(LaneWrapperVector::iterator i = myLaneGeoms.begin();
      i != myLaneGeoms.end(); i++)
  {
    delete(*i);
  }
...
void GUIEdge::initGeometry() {
    if(OptionsCont::getOptions().getInt("verbosity") > 1)
      printf("----> void GUIEdge::initGeometry()\n");
    // don't do this twice
    if(myLaneGeoms.size() > 0) {
        return;
    }
    if(OptionsCont::getOptions().getInt("verbosity") > 1){
      printf("[100] void GUIEdge::initGeometry() - myLaneGeoms...\n");
      for(int i = 0; i < myLaneGeoms.size(); i++)
        printf("[101] void GUIEdge::initGeometry() - myLaneGeoms[%d] = %d\n",i,myLaneGeoms.at(i)->getLinkNumber());
    }
    if(OptionsCont::getOptions().getInt("verbosity") > 1)
      printf("[110] void GUIEdge::initGeometry()...\n");
    // build the lane wrapper
    myLaneGeoms.reserve(myLanes->size());
    if(OptionsCont::getOptions().getInt("verbosity") > 1)
      printf("[120] void GUIEdge::initGeometry()...\n");
    for(unsigned int i = 0; i < myLanes->size(); i++) {
        if(OptionsCont::getOptions().getInt("verbosity") > 1)
          printf("[121] void GUIEdge::initGeometry()...\n");
        // Original was
        myLaneGeoms.push_back(myLanes->at(i)->buildLaneWrapper(i));
        // Within java would be
        //myLaneGeoms.push_back(((GUILane)myLanes->at(i))->buildLaneWrapper(i));
        // So now is
        //myLaneGeoms.push_back((dynamic_cast < GUILane* > (myLanes->at(i)))->buildLaneWrapper(i));
        // because if not, calls the method in MSLane, throwing
    }
    if(OptionsCont::getOptions().getInt("verbosity") > 1)
      printf("<---- void GUIEdge::initGeometry()\n");    
}
...
GUIEdge::drawGL(const GUIVisualizationSettings& s) const {
    ...
    const bool drawEdgeSlope = true; // TASK
    ...
    if(drawEdgeName || drawInternalEdgeName || drawStreetName || drawEdgeSlope)
    {
        GUILaneWrapper* lane1 = myLaneGeoms[0];
        GUILaneWrapper* lane2 = myLaneGeoms[myLaneGeoms.size() - 1];
        Position p = lane1->getShape().positionAtLengthPosition(
         lane1->getShape().length() / (SUMOReal) 2.);
        p.add(lane2->getShape().positionAtLengthPosition(
         lane2->getShape().length() / (SUMOReal) 2.));
        p.mul(.5);
        SUMOReal angle = lane1->getShape().rotationDegreeAtLengthPosition(
         lane1->getShape().length() / (SUMOReal) 2.);
        angle += 90;
        ...
        if (drawEdgeName) {
          if(getID()[0] != ':')
          {
            char *slopeAsString = (char *) malloc(16);
            char *fromJunctionHeightAsString = (char *) malloc(16);
            char *idAsString = (char *) malloc(16);
            char *toJunctionHeightAsString = (char *) malloc(16);
            char *bigCharArray = (char *) malloc(64);
            sprintf(slopeAsString, "%f", mySlope);
            // Es la fromJunction la que viola
            try
            {
              if((const_cast<GUIEdge *>(this))->getFromJunction())
                sprintf(fromJunctionHeightAsString, "%i",
                 (const_cast<GUIEdge *>(this))->getFromJunction()->getHeight());
            }
            catch(...)
            {
              sprintf(fromJunctionHeightAsString, "%s", "???");
            }
            try
            {
              if((const_cast<GUIEdge *>(this))->getToJunction())
                sprintf(toJunctionHeightAsString, "%i",
                 (const_cast<GUIEdge *>(this))->getToJunction()->getHeight());
            }
            catch(...)
            {
              sprintf(fromJunctionHeightAsString, "%s", "???");
            }
            //sprintf(idAsString, "%s", getID());
            strcpy(bigCharArray, fromJunctionHeightAsString);
            strcat(bigCharArray, "-");
            strcat(bigCharArray, slopeAsString);
            strcat(bigCharArray, "-");
            strcat(bigCharArray, idAsString);
            strcat(bigCharArray, "-");
            strcat(bigCharArray, toJunctionHeightAsString);
            GLHelper::drawText(bigCharArray, p, GLO_MAX, s.streetName.size /
             s.scale, s.streetName.color, angle);
            free(slopeAsString);
            free(fromJunctionHeightAsString);
            free(toJunctionHeightAsString);
            free(idAsString);
            free(bigCharArray);
          }
        ...
        else
        {
          ...
          {
          sprintf(slopeAsString,"%f",mySlope);
          GLHelper::drawText(slopeAsString, p, GLO_MAX, s.streetName.size / s.scale,
           s.streetName.color, angle);
          }
        }
        */
        /*
