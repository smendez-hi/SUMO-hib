...
class OutputDevice
{
  public:
    ...
    /**   Dummy function to be able to declare OutputDevice class variables but
     *  without inmediatly initializing them
     */
    static OutputDevice& initializeReference();
     ...
     * @exception IOError If the output could not be built for any reason (error message is supplied)
     */
    static bool createDeviceByOption
     (const std::string& optionName, const std::string& rootElement = "");
     ...
     */
    static OutputDevice& getDeviceByOption
     (const std::string& name)throw(IOError, InvalidArgument);
