...
/// @version $Id: MSVehicle.h 11671 2012-01-07 20:14:30Z behrisch $
///
...
class MSVehicle : public MSBaseVehicle
{
  public:
    ...
    class State
    {
      ...
      public:
        /// Constructor.
        State(SUMOReal pos, SUMOReal speed, SUMOTime lane);
        ...
        /// @name HIB emartin + doancea
        //@{
        SUMOTime getEnterLane() const;

        double getMyPos() const;

        double getMyPosOnLane() const;
        
        double getMyOldPosOnLane() const;

        double getMySpeed() const;

        void setMyPosOnLane(SUMOReal myPos);

        void setEnterLane(SUMOTime enterLane);

        void setMyPos(double myPos);

        void setMySpeed(double mySpeed);

        MSEdge getMyLastEdge() const;

        void setMyLastEdge(MSEdge myLastEdge);

        double getMyEnterPosLane() const;

        void setMyEnterPosLane(double myEnterPosLane);
        
        // TODO TASK FIXME DANGER ALERT Not applyable to nonelectric vehicles
        double getMyLastChargeRemoved() const;
        
        // TODO TASK FIXME DANGER ALERT Not applyable to nonelectric vehicles
        void setMyLastChargeRemoved(double myLastChargeRemoved);
      ...
      private:
        ...
        SUMOReal myPosOnLane;
        
        //SUMOReal myOldPosOnLane;

        SUMOTime myTimeEnterLane;

        SUMOReal myEnterPosLane;

        // TODO TASK FIXME DANGER Not applyable to nonelectric vehicles
        SUMOReal myLastChargeRemoved;
    ...
    MSVehicle
     (SUMOVehicleParameter *pars,const MSRoute *route,const MSVehicleType *type,
      int vehicleIndex/*,bool ecoGemRouter=0*/);
    ...
    bool getFevFlag() { return isFev; }
    
    bool setFevFlag(bool _) { isFev = _; }
    ...
    SUMOReal getCurrentTraveledDistance();
    
    SUMOReal getLastTraveledDistance();
     ...
     */
    inline static SUMOReal gap(SUMOReal predPos, SUMOReal predLength, SUMOReal pos)
    {
     ...
     *
     *   This method goes through all active move reminders, both those for the
     * current lane, stored in "myMoveReminders" and those of prior lanes stored
     * in "myOldLaneMoveReminders" calling "MSMoveReminder::notifyMove".
     *
     ...
     */
    bool moveRegardingCritical
     (SUMOTime t, const MSLane *const lane, const MSVehicle *const pred,
    ...
    SUMOReal getOldPositionOnLane() const
    {
        return myState.myOldPos;
    }    
     ...
     */
    const virtual std::vector<LaneQ> & getBestLanes(bool forceRebuild = false, MSLane *startLane = 0) const;
    ...
    class Influencer
    {
      public:
         ...
         */
        void setConsiderSafeVelocity(bool _) { myConsiderSafeVelocity = _; }
    ...
    State getMyState() const;

    void setMyState(State myState);
    
    State getMyOldState() const;

    MSLane *getMyLane() const;

    bool isHasChangeEdge() const;

    void setHasChangeEdge(bool hasChangeEdge);

    std::string getMyLastEdgeId() const;

    void setMyLastEdgeId(std::string myLastEdgeId);

    void getMySpeed();

    time_t *getLastAdasUpdateTimeT(){return lastAdasUpdateTimeT;}

    void setLastAdasUpdateTimeT(time_t *t){lastAdasUpdateTimeT=t;}
  ...
  protected:
    ...
    const mutable MSEdge *myLastBestLanesEdge;
  ...
  private:
    ...
    {
      MSLink *myLink; SUMOReal myVLinkPass; SUMOReal myVLinkWait;
      bool mySetRequest; SUMOTime myArrivalTime; SUMOReal myArrivalSpeed;
      SUMOReal myDistance;
      DriveProcessItem
       (MSLink *link, double vPass, double vWait, bool setRequest,
        SUMOTime arrivalTime, double arrivalSpeed, double distance)
       :myLink(link), myVLinkPass(vPass), myVLinkWait(vWait),
        mySetRequest(setRequest), myArrivalTime(arrivalTime),
        myArrivalSpeed(arrivalSpeed), myDistance(distance)
      {}
      ;
    };
  ...
  private:
    ...
    void writeFevSyntheticData(MSLane*& lastLane);
    
    void writeNonFevSyntheticData(MSLane*& lastLane);    

    /// @brief invalidated copy constructor
    MSVehicle(const MSVehicle&);

    std::string  myLastEdgeId;
    ...
    void adasWriteExtendedInfo();
    
    void adasWriteGpsString();
    
    std::string adasFriendlyDateStamp(time_t then);
    
    std::string adasFriendlyTimeStamp(time_t then);
    ...
    std::string myWebServiceUrl;
    
    bool isFev;
      
    SUMOReal nonAggregatedEnergyCosts;
    
    SUMOReal myCurrentTraveledDistance;
    
    SUMOReal myLastTraveledDistance;

    time_t *lastAdasUpdateTimeT;
};
