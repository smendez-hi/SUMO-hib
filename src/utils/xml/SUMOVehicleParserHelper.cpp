...
/// @author  Laura Bieker
/// @date    Mon, 07.04.2008
/// @version $Id: SUMOVehicleParserHelper.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
// method definitions
// ===========================================================================
SUMOVehicleParameter* SUMOVehicleParserHelper::parseFlowAttributes
 (const SUMOSAXAttributes& attrs)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>1)
    std::cout << "SUMOVehicleParameter* SUMOVehicleParserHelper::parseFlowAttri"
                 "butes(...)"
              << std::endl;
  ...
  if (attrs.hasAttribute(SUMO_ATTR_PERIOD)
    ...
    throw ProcessError(
        "At most one of '" + attrs.getName(SUMO_ATTR_PERIOD) + "' and '"
  ...
  if (attrs.hasAttribute(SUMO_ATTR_PERIOD)
    ...
    if (attrs.hasAttribute(SUMO_ATTR_END)
        && (attrs.hasAttribute(SUMO_ATTR_NO__DEPRECATED)
            || attrs.hasAttribute(SUMO_ATTR_NUMBER))) {
      throw ProcessError(
          "If '" + attrs.getName(SUMO_ATTR_PERIOD) + "' or '"
              + attrs.getName(SUMO_ATTR_VEHSPERHOUR)
              + "' are given at most one of '"
              + attrs.getName(SUMO_ATTR_END) + "' and '"
              + attrs.getName(SUMO_ATTR_NUMBER)
              + "' are allowed in flow '" + id + "'.");
    }
  } else {
    if (!attrs.hasAttribute(SUMO_ATTR_NO__DEPRECATED)
        && !attrs.hasAttribute(SUMO_ATTR_NUMBER)) {
      throw ProcessError(
  ...
  //TASK This is not verified
  SUMOVehicleParameter* ret = new SUMOVehicleParameter(DEFAULT_VTYPE_ID);
  // Old code was
  //CODE//SUMOVehicleParameter* ret = new SUMOVehicleParameter();
  ...
  if (attrs.hasAttribute(SUMO_ATTR_PERIOD)) {
    ...
    ret->repetitionOffset = attrs.getSUMOTimeReporting(SUMO_ATTR_PERIOD,
        id.c_str(), ok);
#else
  ...
  if (attrs.hasAttribute(SUMO_ATTR_VEHSPERHOUR)) {
    ret->setParameter |= VEHPARS_PERIODFREQ_SET;
    const SUMOReal vph = attrs.getSUMORealReporting(SUMO_ATTR_VEHSPERHOUR,
        id.c_str(), ok);
    if (ok && vph <= 0) {
      delete ret;
      throw ProcessError(
          "Invalid repetition rate in the definition of flow '" + id
              + "'.");
    }
    if (ok && vph != 0) {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_BEGIN)) {
    ret->depart = attrs.getSUMOTimeReporting(SUMO_ATTR_BEGIN, id.c_str(),
        ok);
  }
  if (ok && ret->depart < 0) {
    delete ret;
    throw ProcessError(
        "Negative begin time in the definition of flow '" + id + "'.");
  }
  ...
  if (ok && end <= ret->depart) {
    delete ret;
    throw ProcessError(
        "Flow '" + id + "' ends before or at its begin time.");
  }
  if (attrs.hasAttribute(SUMO_ATTR_NO__DEPRECATED)
      || attrs.hasAttribute(SUMO_ATTR_NUMBER)) {
    ret->repetitionNumber =
        attrs.hasAttribute(SUMO_ATTR_NUMBER) ?
            attrs.getIntReporting(SUMO_ATTR_NUMBER, id.c_str(),
                ok) :
            attrs.getIntReporting(SUMO_ATTR_NO__DEPRECATED,
                id.c_str(), ok);
    if (!gHaveWarnedAboutDeprecatedNumber
        && attrs.hasAttribute(SUMO_ATTR_NO__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedNumber = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_NO__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_NUMBER) + "' instead.");
    }
    ret->setParameter |= VEHPARS_PERIODFREQ_SET;
  ...
  else
  {
    ...
    {
      ret->repetitionNumber = static_cast<int>(static_cast<SUMOReal>(end
          - ret->depart) / ret->repetitionOffset + 0.5);
    }
  }
  if (!ok) {
    delete ret;
...
SUMOVehicleParameter* SUMOVehicleParserHelper::parseVehicleAttributes
 (const SUMOSAXAttributes& attrs, bool skipID, bool skipDepart)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>1)
    std::cout<<"----> void SUMOVehicleParameter* SUMOVehicleParserHelper::parse"
     <<"VehicleAttributes(...)"<<std::endl;
  bool ok = true;
  ...
  if (attrs.hasAttribute(SUMO_ATTR_PERIOD)
      ^ attrs.hasAttribute(SUMO_ATTR_REPNUMBER)) {
    throw ProcessError(
        "The attributes '" + attrs.getName(SUMO_ATTR_PERIOD) + "' and '"
            + attrs.getName(SUMO_ATTR_REPNUMBER)
            + "' have to be given both in the definition of '" + id
            + "'.");
  }
  
  if(oc.getAnyVerbosity()>2)
    std::cout<<"[100] void SUMOVehicleParameter* SUMOVehicleParserHelper::parse"
     <<"VehicleAttributes(...)\n (will now execute... SUMOVehicleParameter* ret"
     <<"=new SUMOVehicleParameter(DEFAULT_VTYPE_ID);)"<<std::endl;
  ...
  if (!skipDepart) {
    const std::string helper = attrs.getStringReporting(SUMO_ATTR_DEPART, 0,
        ok);
    if (helper == "triggered") {
      ret->departProcedure = DEPART_TRIGGERED;
    } else {
      ret->departProcedure = DEPART_GIVEN;
      ret->depart = attrs.getSUMOTimeReporting(SUMO_ATTR_DEPART,
          id.c_str(), ok);
      if (ok && ret->depart < 0) {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_PERIOD)) {
    WRITE_WARNING(
        "period and repno are deprecated in vehicle '" + id + "', use flows instead.");
    ret->setParameter |= VEHPARS_PERIODFREQ_SET;
#ifdef HAVE_SUBSECOND_TIMESTEPS
    ret->repetitionOffset = attrs.getSUMOTimeReporting(SUMO_ATTR_PERIOD,
        id.c_str(), ok);
  ...
  if (attrs.hasAttribute(SUMO_ATTR_REPNUMBER)) {
    ...
    ret->repetitionNumber = attrs.getIntReporting(SUMO_ATTR_REPNUMBER,
        id.c_str(), ok);
  }
...
(const SUMOSAXAttributes& attrs, SUMOVehicleParameter* ret,
  std::string element)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>1)
    std::cout << "void SUMOVehicleParserHelper::parseCommonAttributes(...)"
              << std::endl;
  ...
  if ((attrs.hasAttribute(SUMO_ATTR_FROM_TAZ)
      || attrs.hasAttribute(SUMO_ATTR_FROM_TAZ__DEPRECATED))
      && (attrs.hasAttribute(SUMO_ATTR_TO_TAZ)
          || attrs.hasAttribute(SUMO_ATTR_TO_TAZ__DEPRECATED))) {
    ret->setParameter |= VEHPARS_TAZ_SET;
    ret->fromTaz =
        attrs.hasAttribute(SUMO_ATTR_FROM_TAZ) ?
            attrs.getStringReporting(SUMO_ATTR_FROM_TAZ, 0, ok) :
            attrs.getStringReporting(SUMO_ATTR_FROM_TAZ__DEPRECATED,
                0, ok);
    ret->toTaz =
        attrs.hasAttribute(SUMO_ATTR_TO_TAZ) ?
            attrs.getStringReporting(SUMO_ATTR_TO_TAZ, 0, ok) :
            attrs.getStringReporting(SUMO_ATTR_TO_TAZ__DEPRECATED,
                0, ok);
    if (!gHaveWarnedAboutDeprecatedTazs
        && (attrs.hasAttribute(SUMO_ATTR_TO_TAZ__DEPRECATED)
            || attrs.hasAttribute(SUMO_ATTR_FROM_TAZ__DEPRECATED))) {
      gHaveWarnedAboutDeprecatedTazs = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_FROM_TAZ__DEPRECATED) + "'/'" + toString(SUMO_ATTR_TO_TAZ__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_FROM_TAZ) + "'/'" + toString(SUMO_ATTR_TO_TAZ) + "' instead.");
    }
  }
  ...
  if (attrs.hasAttribute(SUMO_ATTR_DEPARTLANE)
      || attrs.hasAttribute(SUMO_ATTR_DEPARTLANE__DEPRECATED)) {
    ret->setParameter |= VEHPARS_DEPARTLANE_SET;
    const std::string helper =
        attrs.hasAttribute(SUMO_ATTR_DEPARTLANE) ?
            attrs.getStringReporting(SUMO_ATTR_DEPARTLANE, 0, ok) :
            attrs.getStringReporting(
                SUMO_ATTR_DEPARTLANE__DEPRECATED, 0, ok);
    if (!gHaveWarnedAboutDeprecatedDepartLane
        && attrs.hasAttribute(SUMO_ATTR_DEPARTLANE__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedDepartLane = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_DEPARTLANE__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_DEPARTLANE) + "' instead.");
    }
    if (helper == "random") {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_DEPARTPOS)
      || attrs.hasAttribute(SUMO_ATTR_DEPARTPOS__DEPRECATED)) {
    ret->setParameter |= VEHPARS_DEPARTPOS_SET;
    const std::string helper =
        attrs.hasAttribute(SUMO_ATTR_DEPARTPOS) ?
            attrs.getStringReporting(SUMO_ATTR_DEPARTPOS, 0, ok) :
            attrs.getStringReporting(
                SUMO_ATTR_DEPARTPOS__DEPRECATED, 0, ok);
    if (!gHaveWarnedAboutDeprecatedDepartPos
        && attrs.hasAttribute(SUMO_ATTR_DEPARTPOS__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedDepartPos = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_DEPARTPOS__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_DEPARTPOS) + "' instead.");
    }
    if (helper == "random") {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_DEPARTSPEED)
      || attrs.hasAttribute(SUMO_ATTR_DEPARTSPEED__DEPRECATED)) {
    ret->setParameter |= VEHPARS_DEPARTSPEED_SET;
    std::string helper =
        attrs.hasAttribute(SUMO_ATTR_DEPARTSPEED) ?
            attrs.getStringReporting(SUMO_ATTR_DEPARTSPEED, 0, ok) :
            attrs.getStringReporting(
                SUMO_ATTR_DEPARTSPEED__DEPRECATED, 0, ok);
    if (!gHaveWarnedAboutDeprecatedDepartSpeed
        && attrs.hasAttribute(SUMO_ATTR_DEPARTSPEED__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedDepartSpeed = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_DEPARTSPEED__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_DEPARTSPEED) + "' instead.");
    }
    if (helper == "random") {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_ARRIVALLANE)
      || attrs.hasAttribute(SUMO_ATTR_ARRIVALLANE__DEPRECATED)) {
    ret->setParameter |= VEHPARS_ARRIVALLANE_SET;
    std::string helper =
        attrs.hasAttribute(SUMO_ATTR_ARRIVALLANE) ?
            attrs.getStringReporting(SUMO_ATTR_ARRIVALLANE, 0, ok) :
            attrs.getStringReporting(
                SUMO_ATTR_ARRIVALLANE__DEPRECATED, 0, ok);
    if (!gHaveWarnedAboutDeprecatedArrivalLane
        && attrs.hasAttribute(SUMO_ATTR_ARRIVALLANE__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedArrivalLane = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_ARRIVALLANE__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_ARRIVALLANE) + "' instead.");
    }
    if (helper == "current") {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_ARRIVALPOS)
      || attrs.hasAttribute(SUMO_ATTR_ARRIVALPOS__DEPRECATED)) {
    ret->setParameter |= VEHPARS_ARRIVALPOS_SET;
    std::string helper =
        attrs.hasAttribute(SUMO_ATTR_ARRIVALPOS) ?
            attrs.getStringReporting(SUMO_ATTR_ARRIVALPOS, 0, ok) :
            attrs.getStringReporting(
                SUMO_ATTR_ARRIVALPOS__DEPRECATED, 0, ok);
    if (!gHaveWarnedAboutDeprecatedArrivalPos
        && attrs.hasAttribute(SUMO_ATTR_ARRIVALPOS__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedArrivalPos = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_ARRIVALPOS__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_ARRIVALPOS) + "' instead.");
    }
    if (helper == "random") {
  ...
  if (attrs.hasAttribute(SUMO_ATTR_ARRIVALSPEED)
      || attrs.hasAttribute(SUMO_ATTR_ARRIVALSPEED__DEPRECATED)) {
    ret->setParameter |= VEHPARS_ARRIVALSPEED_SET;
    std::string helper =
        attrs.hasAttribute(SUMO_ATTR_ARRIVALSPEED) ?
            attrs.getStringReporting(SUMO_ATTR_ARRIVALSPEED, 0,
                ok) :
            attrs.getStringReporting(
                SUMO_ATTR_ARRIVALSPEED__DEPRECATED, 0, ok);
    if (!gHaveWarnedAboutDeprecatedArrivalSpeed
        && attrs.hasAttribute(SUMO_ATTR_ARRIVALSPEED__DEPRECATED)) {
      gHaveWarnedAboutDeprecatedArrivalSpeed = true;
      WRITE_WARNING(
          "'" + toString(SUMO_ATTR_ARRIVALSPEED__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_ARRIVALSPEED) + "' instead.");
    }
    if (helper == "current") {
      ret->arrivalSpeedProcedure = ARRIVAL_SPEED_CURRENT;
    } else {
      try {
        ret->arrivalSpeed = TplConvert<char>::_2SUMOReal(
            helper.c_str());
  ...
  if (attrs.hasAttribute(SUMO_ATTR_COLOR)) {
    ...
    try {
      ret->color = RGBColor::parseColor(
          attrs.getStringReporting(SUMO_ATTR_COLOR, 0, ok));
    } catch (NumberFormatException&) {
      throw ProcessError(
          "Invalid color definition for " + element + " '" + ret->id
              + "'");
    } catch (EmptyData&) {
      throw ProcessError(
          "Invalid color definition for " + element + " '" + ret->id
...
SUMOVTypeParameter* SUMOVehicleParserHelper::beginVTypeParsing
 (const SUMOSAXAttributes& attrs)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>1)
    std::cout<<"----> SUMOVTypeParameter* SUMOVehicleParserHelper::beginVTypePa"
     <<"rsing(const SUMOSAXAttributes& attrs)"<<std::endl;
  if(oc.getAnyVerbosity()>2)
    std::cout<<"[100] SUMOVTypeParameter* SUMOVehicleParserHelper::beginVTypePa"
     <<"rsing(const SUMOSAXAttributes& attrs)"<<std::endl<<" (will call now SUM"
     <<"OVTypeParameter* vtype=new SUMOVTypeParameter(DEFAULT_VTYPE_ID);"
     <<std::endl;              
#ifndef SUMO_MINT
  // New code:
  bool ok=true;
  std::string typeId=attrs.getStringReporting(SUMO_ATTR_ID, 0, ok);
  SUMOVTypeParameter* vtype=new SUMOVTypeParameter(typeId);  
#else
  // Original code was:
  ...
  vtype->id=attrs.getStringReporting(SUMO_ATTR_ID, 0, ok);
	...
	if (attrs.hasAttribute(SUMO_ATTR_LENGTH)) {
		...
		vtype->length = attrs.getSUMORealReporting(SUMO_ATTR_LENGTH,
				vtype->id.c_str(), ok);
		vtype->setParameter |= VTYPEPARS_LENGTH_SET;
	}
	if (attrs.hasAttribute(SUMO_ATTR_MINGAP)) {
		vtype->minGap = attrs.getSUMORealReporting(SUMO_ATTR_MINGAP,
				vtype->id.c_str(), ok);
	...
	if (attrs.hasAttribute(SUMO_ATTR_GUIOFFSET)) {
		...
		vtype->minGap = attrs.getSUMORealReporting(SUMO_ATTR_GUIOFFSET,
				vtype->id.c_str(), ok);
		vtype->setParameter |= VTYPEPARS_MINGAP_SET;
	}
	if (attrs.hasAttribute(SUMO_ATTR_MAXSPEED)) {
		vtype->maxSpeed = attrs.getSUMORealReporting(SUMO_ATTR_MAXSPEED,
				vtype->id.c_str(), ok);
		vtype->setParameter |= VTYPEPARS_MAXSPEED_SET;
	} else if (attrs.hasAttribute(SUMO_ATTR_MAXSPEED__DEPRECATED)) {
		vtype->maxSpeed = attrs.getSUMORealReporting(
				SUMO_ATTR_MAXSPEED__DEPRECATED, vtype->id.c_str(), ok);
		vtype->setParameter |= VTYPEPARS_MAXSPEED_SET;
		if (!gHaveWarnedAboutDeprecatedMaxSpeed) {
			gHaveWarnedAboutDeprecatedMaxSpeed = true;
			WRITE_WARNING(
					"'" + toString(SUMO_ATTR_MAXSPEED__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_MAXSPEED) + "' instead.");
		}
	}
	if (attrs.hasAttribute(SUMO_ATTR_SPEEDFACTOR)) {
		vtype->speedFactor = attrs.getSUMORealReporting(SUMO_ATTR_SPEEDFACTOR,
				vtype->id.c_str(), ok);
		vtype->setParameter |= VTYPEPARS_SPEEDFACTOR_SET;
	}
	if (attrs.hasAttribute(SUMO_ATTR_SPEEDDEV)) {
		vtype->speedDev = attrs.getSUMORealReporting(SUMO_ATTR_SPEEDDEV,
				vtype->id.c_str(), ok);
	...
	if (attrs.hasAttribute(SUMO_ATTR_VCLASS)
			|| attrs.hasAttribute(SUMO_ATTR_VCLASS__DEPRECATED)) {
		if (!gHaveWarnedAboutDeprecatedVClass
				&& attrs.hasAttribute(SUMO_ATTR_VCLASS__DEPRECATED)) {
			gHaveWarnedAboutDeprecatedVClass = true;
			WRITE_WARNING(
					"'" + toString(SUMO_ATTR_VCLASS__DEPRECATED) + "' is deprecated, please use '" + toString(SUMO_ATTR_VCLASS) + "' instead.");
		}
	...
	if (attrs.hasAttribute(SUMO_ATTR_GUIWIDTH)) {
		vtype->width = attrs.getSUMORealReporting(SUMO_ATTR_GUIWIDTH,
				vtype->id.c_str(), ok);
	...
	if (attrs.hasAttribute(SUMO_ATTR_COLOR)) {
		vtype->color = RGBColor::parseColorReporting(
				attrs.getString(SUMO_ATTR_COLOR), attrs.getObjectType(),
				vtype->id.c_str(), true, ok);
		vtype->setParameter |= VTYPEPARS_COLOR_SET;
	} else {
		vtype->color = RGBColor(1, 1, 0);
	}
	if (attrs.hasAttribute(SUMO_ATTR_PROB)) {
		vtype->defaultProbability = attrs.getSUMORealReporting(SUMO_ATTR_PROB,
				vtype->id.c_str(), ok);
  ...
  {
    vtype->mass=attrs.getSUMORealReporting(SUMO_ATTR_MASS,vtype->id.c_str(),ok);
  }
	if (attrs.hasAttribute(SUMO_ATTR_ROLLING_RES)) {
		vtype->rolling_res = attrs.getSUMORealReporting(SUMO_ATTR_ROLLING_RES,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_AIR_DENSITY)) {
		vtype->air_dens = attrs.getSUMORealReporting(SUMO_ATTR_AIR_DENSITY,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_FRONTAL_AREA)) {
		vtype->frontal_area = attrs.getSUMORealReporting(SUMO_ATTR_FRONTAL_AREA,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_DRAG_COEFFICIENT)) {
		vtype->drag_coefficient = attrs.getSUMORealReporting(
				SUMO_ATTR_DRAG_COEFFICIENT, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_GEAR_RATIO)) {
		vtype->drag_coefficient = attrs.getSUMORealReporting(
				SUMO_ATTR_GEAR_RATIO, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_GEAR_SYS_EFFC)) {
		vtype->drag_coefficient = attrs.getSUMORealReporting(
				SUMO_ATTR_GEAR_SYS_EFFC, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_REGEN_RATIO)) {
		vtype->regen_ratio = attrs.getSUMORealReporting(SUMO_ATTR_REGEN_RATIO,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_MOTOR_SYS_EFFC)) {
		vtype->motor_sys_effc = attrs.getSUMORealReporting(
				SUMO_ATTR_MOTOR_SYS_EFFC, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_COOPER_LOSSES)) {
		vtype->cooper_losses = attrs.getSUMORealReporting(
				SUMO_ATTR_COOPER_LOSSES, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_IRON_LOSSES)) {
		vtype->iron_losses = attrs.getSUMORealReporting(SUMO_ATTR_IRON_LOSSES,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_WINDAGE_LOSSES)) {
		vtype->windage_losses = attrs.getSUMORealReporting(
				SUMO_ATTR_WINDAGE_LOSSES, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_CONSTANT_LOSSES)) {
		vtype->constant_losses = attrs.getSUMORealReporting(
				SUMO_ATTR_WINDAGE_LOSSES, vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_POWER_ACCS)) {
		vtype->power_accs = attrs.getSUMORealReporting(SUMO_ATTR_POWER_ACCS,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_NUM_CELLS)) {
		vtype->num_cells = attrs.getSUMORealReporting(SUMO_ATTR_NUM_CELLS,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_CAPACITY)) {
		vtype->capacity = attrs.getSUMORealReporting(SUMO_ATTR_CAPACITY,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_TIME_CHARGE)) {
		vtype->time_charge = attrs.getSUMORealReporting(SUMO_ATTR_TIME_CHARGE,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_ENV_HUM)) {
		vtype->humidity = attrs.getSUMORealReporting(SUMO_ATTR_ENV_HUM,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_ENV_TEMP)) {
		vtype->celsius = attrs.getSUMORealReporting(SUMO_ATTR_ENV_TEMP,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_AUX_MASK)) {
		vtype->aux_mask = attrs.getStringReporting(SUMO_ATTR_AUX_MASK,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_LIGHT_MASK)) {
		vtype->light_mask = attrs.getStringReporting(SUMO_ATTR_LIGHT_MASK,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_HEAD_LIGHT)) {
		vtype->vehicle_headlights_consumption = attrs.getSUMORealReporting(SUMO_ATTR_HEAD_LIGHT,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_FOG_LIGHT)) {
		vtype->front_fog_lights_consumption = attrs.getSUMORealReporting(SUMO_ATTR_FOG_LIGHT,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_COCKPIT_LIGHT)) {
		vtype->cockpit_lights_consumption = attrs.getSUMORealReporting(SUMO_ATTR_COCKPIT_LIGHT,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_RADIO)) {
		vtype->radio_consumption = attrs.getSUMORealReporting(SUMO_ATTR_RADIO,
				vtype->id.c_str(), ok);
	}
	if (attrs.hasAttribute(SUMO_ATTR_HEATING_AC)) {
		vtype->heatingAC_consumption = attrs.getSUMORealReporting(SUMO_ATTR_HEATING_AC,
				vtype->id.c_str(), ok);
	}
	try {
...
void SUMOVehicleParserHelper::parseVTypeEmbedded
 (SUMOVTypeParameter& into, int element, const SUMOSAXAttributes& attrs,
  bool fromVType)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>1)
    std::cout << "void SUMOVehicleParserHelper::parseVTypeEmbedded(...)"
              << std::endl;
	...
	if (cf_it == allowedAttrs.end()) {
		if (SUMOXMLDefinitions::Tags.has(element)) {
			WRITE_WARNING(
					"Unknown cfmodel " + toString((SumoXMLTag)element) + " when parsing vtype '" + into.id + "'");
		} else {
			WRITE_WARNING(
					"Unknown cfmodel when parsing vtype '" + into.id + "'");
		}
	}
	if (!fromVType) {
	...
	bool ok = true;
	for (std::set<SumoXMLAttr>::const_iterator it = cf_it->second.begin();
			it != cf_it->second.end(); it++) {
		if (attrs.hasAttribute(*it)) {
			into.cfParameter[*it] = attrs.getSUMORealReporting(*it,
					into.id.c_str(), ok);
		}
	}
	if (!ok) {
...
SUMOVehicleClass SUMOVehicleParserHelper::parseVehicleClass
 (const SUMOSAXAttributes& attrs, const std::string& id)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>1)
    std::cout << "SUMOVehicleClass SUMOVehicleParserHelper::parseVehicleClass(."
                 "..)"
              << std::endl;
	...
	try {
		bool ok = true;
		std::string vclassS =
				attrs.hasAttribute(SUMO_ATTR_VCLASS) ?
						attrs.getOptStringReporting(SUMO_ATTR_VCLASS,
								id.c_str(), ok, "") :
						attrs.getOptStringReporting(
								SUMO_ATTR_VCLASS__DEPRECATED, id.c_str(), ok,
								"");
		if (vclassS == "") {
			return vclass;
		}
		...
		WRITE_ERROR(
				"The class for " + attrs.getObjectType() + " '" + id + "' is not known.");
	}
	return vclass;
}

SUMOEmissionClass SUMOVehicleParserHelper::parseEmissionClass(
		const SUMOSAXAttributes& attrs, const std::string& id) {
	SUMOEmissionClass vclass = SVE_UNKNOWN;
	try {
		bool ok = true;
		std::string vclassS = attrs.getOptStringReporting(
				SUMO_ATTR_EMISSIONCLASS, id.c_str(), ok, "");
		if (vclassS == "") {
			return vclass;
		}
		...
		WRITE_ERROR(
				"The emission class for " + attrs.getObjectType() + " '" + id + "' is not known.");
	}
	return vclass;
}

SUMOVehicleShape SUMOVehicleParserHelper::parseGuiShape(
		const SUMOSAXAttributes& attrs, const std::string& id) {
	bool ok = true;
	std::string vclassS = attrs.getOptStringReporting(SUMO_ATTR_GUISHAPE,
			id.c_str(), ok, "");
	if (SumoVehicleShapeStrings.hasString(vclassS)) {
		return SumoVehicleShapeStrings.get(vclassS);
	} else {
		WRITE_ERROR(
				"The shape '" + vclassS + "' for " + attrs.getObjectType() + " '" + id + "' is not known.");
		return SVS_UNKNOWN;
	}
}
