...
/// @date    Sept 2002
/// @version $Id: GUIVehicle.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
GUIVehicle::GUIVehicle
 (SUMOVehicleParameter* pars, const MSRoute* route,
  const MSVehicleType* type, int vehicleIndex/*, bool ecoGemRouter*/)
 :MSVehicle(pars, route, type, vehicleIndex/*, ecoGemRouter*/),
  GUIGlObject(GLO_VEHICLE, pars->id), oc(OptionsCont::getOptions())
{
  /* as it is possible to show all vehicle routes, we have to store them...
   * (bug [ 2519761 ])
   */
  if(oc.getAnyVerbosity()>1)
  {
    std::cout << "----> GUIVehicle::GUIVehicle(...)\n"
              << "  pars->color=" << pars->color << "\n"
              << "  pars->id=" << pars->id << std::endl ;
  }
...
{
  if(oc.getAnyVerbosity()>1)
   std::cout << "----> GUIVehicle::~GUIVehicle()" << std::endl;
    myLock.lock();
...
GUIParameterTableWindow* GUIVehicle::getParameterWindow
 (GUIMainWindow& app,GUISUMOAbstractView&)
{
...
inline void drawAction_drawVehicleAsBoxPlus(const GUIVehicle& veh)
{
  glPushMatrix();
...
inline void drawAction_drawVehicleAsTrianglePlus(const GUIVehicle& veh)
{
  if(OptionsCont::getOptions().getAnyVerbosity()>1)
  {
    std::cout << "----> inline void drawAction_drawVehicleAsTrianglePlus(...)\n"
              << "  id=" << veh.getID() << std::endl;
  }  
  if(veh.getVehicleType().getID()==DEFAULT_ELECVTYPE_ID)
  {
    GLHelper::setColor(HIB_CAR_COLOR_GREEN);
  }
  else
  {
    /* Must be equal to DEFAULT_VTYPE_ID
     * and the color already set somewhy to HIB_CAR_COLOR_YELLOW
     */
    GLHelper::setColor(HIB_CAR_COLOR_YELLOW_TRY);
  }      
...
inline void drawAction_drawVehicleAsPoly(const GUIVehicle& veh)
{
  if(OptionsCont::getOptions().getAnyVerbosity() > 1)
  {
    std::cout << "----> inline void drawAction_drawVehicleAsPoly(...)\n"
              << "  id=" << veh.getID() << std::endl;
  }
    RGBColor current = GLHelper::getColor();
  /* Remember veh.getVehicleType().getID() != veh.getID() */
  if(veh.getVehicleType().getID()==DEFAULT_ELECVTYPE_ID)
  {
    current = HIB_CAR_COLOR_GREEN;
  }
  else
  {
    /* Must be equal to DEFAULT_VTYPE_ID
  ...
  if(OptionsCont::getOptions().getAnyVerbosity() > 1)
  {
    std::cout << "[100] inline void drawAction_drawVehicleAsPoly(...)\n"
              << "  shape=" << shape << "\n"
              << "  lighter=" << lighter << std::endl;
  }    
  /* Float rnd can not be declared inside the 'switch' selective sent. */
  float rnd;
  // draw main body
  switch (shape)
  {
...
void GUIVehicle::drawGL
 (const GUIVisualizationSettings& s) const
{
 if(oc.getAnyVerbosity() > 1)
  std::cout << "----> void GUIVehicle::drawGL(const ...) const" << std::endl;
    glPushName(getGlID());
    glPushMatrix();
...
void GUIVehicle::drawGLAdditional
 (GUISUMOAbstractView* const parent, const GUIVisualizationSettings& s) const
{
 if(oc.getAnyVerbosity() > 1)
  std::cout << "----> void GUIVehicle::drawGLAdditional(...) const"
            << std::endl;
    glPushName(getGlID());
    glPushMatrix();
