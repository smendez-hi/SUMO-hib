...
/// @version $Id: OptionsCont.cpp 11672 2012-01-07 20:29:23Z behrisch $
///
    ...
    : myAddresses(), myValues(), myDeprecatedSynonymes(), myHaveInformedAboutDeprecatedDivider(false)
{
  ...
  loadVerbosity = buildVerbosity = simulationVerbosity = anyVerbosity = -1;
}
...
bool OptionsCont::isSet(const std::string& name, bool failOnNonExistant) const
{
...
bool OptionsCont::getSafeBool(const std::string& name) const
{
  Option *o;
  try
  {
  ...
  // Here is safe to retrieve getSecure(name)
  o = getSecure(name);
  //   This will return the boolean value, or will throw InvalidArgument
  // if not a boolean option
  return o->getBool();
}
...
OptionsCont::set(const std::string& name, const std::string& value) {
    Option* o = getSecure(name);
   /* std::cout << "Get Option description: " << o->getDescription() << "\n";
    std::cout << "Get Option string: " << o->getString() << "\n";*/
   /* std::cout << "Get the simulation file: "<<getString("extra-file").c_str()<< "\n";*/
    if (!o->isWriteable()) {
...
OptionsCont::addDescription(const std::string& name,
    ...
    Option* o = getSecure(name);
    assert(o != 0);
    assert(find(mySubTopics.begin(), mySubTopics.end(), subtopic) != 
           mySubTopics.end());
...
const std::string OptionsCont::getAppName()const{return myAppName;}
...
void OptionsCont::splitLines
 (std::ostream& os, std::string what, size_t offset, size_t nextOffset)
{
    while (what.length() > 0) {
...
OptionsCont::writeConfiguration(std::ostream& os, bool filled,
    ...
    // This one was the non hib schema
    //os << "<configuration xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:noNamespaceSchemaLocation=\"http://sumo.sf.net/xsd/sumoConfiguration.xsd\">" << std::endl << std::endl;
    // This another, the new hib schema
    os << "<configuration xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:noNamespaceSchemaLocation=\"sumoHibConfiguration.xsd\">" << std::endl << std::endl;
...
int OptionsCont::getSimulationVerbosity()
{
  if(simulationVerbosity==-1)
  {
    if(getOptions().getInt("allVerbosities"))
      simulationVerbosity =
        max2(getOptions().getInt("allVerbosities"),getOptions().getInt("simulationVerbosity"));
    else
      simulationVerbosity = getOptions().getInt("simulationVerbosity");
  }
  return simulationVerbosity;
}
    
int OptionsCont::getLoadVerbosity()
{
  if(loadVerbosity==-1)
  {
    if(getOptions().getInt("allVerbosities"))
      loadVerbosity =
        max2(getOptions().getInt("allVerbosities"),getOptions().getInt("loadVerbosity"));
    else
      loadVerbosity = getOptions().getInt("loadVerbosity");
  }
  return loadVerbosity;
}

int OptionsCont::getBuildVerbosity()
{
  if(buildVerbosity==-1)
  {
    if(getOptions().getInt("allVerbosities"))
      buildVerbosity =
        max2(getOptions().getInt("allVerbosities"),getOptions().getInt("buildVerbosity"));
    else
      buildVerbosity = getOptions().getInt("buildVerbosity");
  }
  return buildVerbosity;
}

int OptionsCont::getAnyVerbosity()
{
  if(anyVerbosity==-1)
    anyVerbosity = 
     max2(getOptions().getInt("allVerbosities"),
      max2(getOptions().getInt("loadVerbosity"),
       max2(getOptions().getInt("buildVerbosity"),
        getOptions().getInt("simulationVerbosity"))));
  return anyVerbosity;
}
