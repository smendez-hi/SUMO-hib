...
/// @version $Id: OptionsCont.h 11671 2012-01-07 20:14:30Z behrisch $
///
...
class OptionsCont
{
 public:
  ...
  /** @brief Returns the boolean-value of the named option (only for Option_Bool)
    *
    *   This method returns the boolean-value of a boolean-option, if exists.
    * If the named option is not a boolean-option, an InvalidArgument is
    * thrown. If the named option does not exist, false is returned
    * @param[in] name The name of the option to return the boolean-value of
    * @return The boolean-value of the named, existing boolean-option
    * @exception InvalidArgument If the option is not a boolean-option
    */
  bool getSafeBool(const std::string& name) const;
  ...
  /** @brief Checks for help and configuration output, returns whether we should exit
    *
    *   Returns false if no error was detected and the application may be
    * executed (at least from this point of view). If missingOptions is true,
    * the user is
    * informed that they should be supplied (returns true). Otherwise it is
    * checked
    * whether help shall be printed what is done if so, returning true. Also,
    * it
  ...
  std::vector<std::string> getSubTopicsEntries
  ...
  int getSimulationVerbosity();
  
  int getLoadVerbosity();
  
  int getBuildVerbosity();
 ...
 private:
  ...
  /** definition of the type that realizes the access to options */
  ...
  /**   A map from deprecated options to a bool indicating whether we warned
    *  about deprecation
    */
