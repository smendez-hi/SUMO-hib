...
/// @date    Tue, 31.03.2009
/// @version $Id: SUMOVehicleParameter.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
SUMOVehicleParameter::SUMOVehicleParameter
 ...
 :vtypeid(vTypeId), color(RGBColor::DEFAULT_COLOR), depart(-1),
  departProcedure(DEPART_GIVEN), departLane(0),
  departLaneProcedure(DEPART_LANE_DEFAULT), departPos(0),
  departPosProcedure(DEPART_POS_DEFAULT), departSpeed(-1),
  departSpeedProcedure(DEPART_SPEED_DEFAULT), arrivalLane(0), 
  arrivalLaneProcedure(ARRIVAL_LANE_DEFAULT), arrivalPos(0),
  arrivalPosProcedure(ARRIVAL_POS_DEFAULT), arrivalSpeed(-1),
  arrivalSpeedProcedure(ARRIVAL_SPEED_DEFAULT), repetitionNumber(-1),
  repetitionsDone(-1), repetitionOffset(-1), line(), fromTaz(), toTaz(),
  setParameter(0)
{
  OptionsCont &oc=OptionsCont::getOptions();
  if(oc.getAnyVerbosity()>2)
    std::cout<<"----> SUMOVehicleParameter::SUMOVehicleParameter(const std::str"
     <<"ing vTypeId{"<<vTypeId<<"})"<<std::endl;
}
...
bool SUMOVehicleParameter::arrivalposValidate(const std::string& val) {
    if (val == "random" || val == "max") {
        return true;
    }
    try {
...
bool SUMOVehicleParameter::arrivalspeedValidate(const std::string& val) {
    if (val == "current") {
        return true;
    }
    try {
