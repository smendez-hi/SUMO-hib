...
bool MSVehicle::moveChecked()
{
  ...
  if(aux1)
    aux1->updateAuxiliaryServicesOnOffStatus();
  ...
  if(aux1)
  {
    ...
    if(oc.getSimulationVerbosity() > 2)
    {
      ...
      std::cout<<" maxSpeedByTorque{"<<maxSpeedByTorque<<"}"<<std::endl;
  ...
  if(aux1)
  {
    if(oc.getSimulationVerbosity()>2)
    {
      std::cout<<"[425] bool MSVehicle::moveChecked()"<<std::endl;
      std::cout<<" vNext{"<<vNext<<"}"<<std::endl;
      std::cout<<" getMaxSpeed(){"<<getMaxSpeed()<<"}"<<std::endl;
      std::cout<<" maxSpeedByTorque{"<<maxSpeedByTorque<<"}"<<std::endl;
    }
    // Electric vehicle
  ...
  if(oc.getSimulationVerbosity()>2)
  {
    std::cout<<"[450] bool MSVehicle::moveChecked()"<<std::endl;    
    std::cout<<" vNext{"<<vNext<<"}"<<std::endl;
  }
          ...
          tag1:
            ...
            if(!amAdasChecked)
            {
              if      (strcmp(getID().c_str(),"fev0")==0){
                ...
                myWebServiceUrl="http://147.102.7.35:8792/VDAdas?wsdl";
          ...
          (strcmp(getID().c_str(),"fev1")==0))
        {
          if(!amAdasChecked)
          {
            ...
            if      (strcmp(getID().c_str(),"fev0")==0){ // ECOGEM MODE
              ...
              // GREECE
              //myWebServiceUrl="http://147.102.7.30:8792/VDAdas?wsdl";
              myWebServiceUrl="http://147.102.7.35:8792/VDAdas?wsdl"; // FIXME
...
void MSVehicle::adasWriteGpsString()
{
  ...
  */
  ...
  /* FIXME IF CITY IS CHIERI TESTAREA */
  if(MSNet::getInstance()->getCity() == "cambiano" ||
   MSNet::getInstance()->getCity() == "Cambiano")
  {
    sprintf(aux,"00%8.4f,",longitude);
  }
  /* FIXME IF CITY IS MUNICH TESTAREA */
  else if(MSNet::getInstance()->getCity() == "munich" ||
   MSNet::getInstance()->getCity() == "Munich" ||
   MSNet::getInstance()->getCity() == "munchen" ||
   MSNet::getInstance()->getCity() == "Munchen")
  {
    sprintf(aux,"0%8.4f,",longitude);
  }
  else
    /* German test site by default */
    sprintf(aux,"0%8.4f,",longitude);
  gprmcString += aux;
  ...
  */
  ...
  /* 084.4        Track angle in degrees True */
  angle = getAngle();
  /* angle ranges from -180 ('left' north) to 180 ('right' north)
   * bearing ranges from 0 ('left' north) to 360 ('right' north)
   */
  ...
  if(oc.getSimulationVerbosity() > 2)
  {
    ...
    std::cout << " angle{" << angle << "}, bearing{" << bearing << "}"
     << std::endl;
  }
  gprmcString += ",";
  ...
  */
  ...
  command = "python sudsVdadasPresendWithoutChecksum.py -i " + gprmcString;
...
void MSVehicle::adasWriteExtendedInfo()
{
  ...
  std::string
   AuxStatus="0", // TODO
   BatteryCapacity =
    OutputDevice::realString(getMyType()->getCapacity() * 
    getMyType()->getNumCells(),2),
   ...
   EnvHumidity=OutputDevice::realString(MSNet::getInstance()->getCurrentEnvHum(),2),
   //EnvTemperature=OutputDevice::realString(getMyType()->getCelsius(),2),
   EnvTemperature=OutputDevice::realString(MSNet::getInstance()->getCurrentEnvTemp(),2),
   Ignition="0", // TODO
   IsCharging="0", // TODO
   Power="0", // TODO
   VIN=getID(),
   VehicleWeight=OutputDevice::realString(getMyType()->getMass(),2);
  std::string osOrder="python sudsVdadasPresendExtInfo.py ";
...
void MSVehicle::writeFevSyntheticData(MSLane*& lastLane)
{
  ...
  try
  {
...
#else
    {
      ...
      if (isOnRoad()) {
          ...
          // Watts = Amps x Volts
          double mean_energy_removed_by_edge = ampheres_spent_segment
            * aux1->getMyCurrE();
          /* Segfault may has happened above */
          
          //avg_speed = getMySpeed(); // TODO TASK FIXME UNIMPLEMENTED?
          avg_speed = myState.getMySpeed();
          ...
          if (false) {
              mean_energy_removed_by_edge *=1;
          }

          // Driver type: Normal
          if (false) {
              mean_energy_removed_by_edge *= 1.1;
          }

          // Driver type: Aggressive
          if (false) {
              mean_energy_removed_by_edge *= 1.3;
          }

          //Adds aggregated energy costs.
          aggredatedEnergyCosts += mean_energy_removed_by_edge;
          nonAggregatedEnergyCosts += mean_energy_removed_by_edge;
          
          // Build AuxSvcsStatus
          int auxSvcsStatus = 0;
          auxSvcsStatus |= aux1->getCac();
          auxSvcsStatus |= aux1->getCch() << 2;
          auxSvcsStatus |= aux1->getCca() << 4;
          auxSvcsStatus |= aux1->getCvhl() << 8;
          auxSvcsStatus |= aux1->getCffl() << 10;
          auxSvcsStatus |= aux1->getCcl() << 11;

          //Makes addition of the energy costs. ???
	  
	  // (smendez) March 10, 2015. AuxStatus as in D3.3 v17 page 34
	  int auxStatusEcoGem = 0;
	  auxStatusEcoGem |= 1 << 0; // b0: heater status available (supossed is available always)
	  auxStatusEcoGem |= 1 << 2; // b2: cooler status available (supossed is available always)
	  auxStatusEcoGem |= 1 << 4; // b4: braking pedal status available (supossed is available always)
	  auxStatusEcoGem |= 1 << 6; // b6: accelerator pedal status available (supossed is available always)
	  if (aux1->getMyCurrSpeed() - aux1->getMyLastSpeed() > 0) {
	    auxStatusEcoGem |= 1 << 7; // b7: accelerating
	    auxStatusEcoGem |= 0 << 5;
	  } else {
	    auxStatusEcoGem |= 1 << 5; // b5: braking
	    auxStatusEcoGem |= 1 << 7;
	  }
	  if (aux1->getCch() == 0) { // TODO Preguntar a Uxio qué son estos valores, porque no van de 0 a 3
	    auxStatusEcoGem |= 0 << 1; // b1: heater status
	  } else {
	    auxStatusEcoGem |= 1 << 1;
	  }
	  if (aux1->getCac() == 0) {
	    auxStatusEcoGem |= 0 << 3; // b3: cooler status
	  } else {
	    auxStatusEcoGem |= 1 << 3;
	  }
	  
          of
            /*<<" AuxStatus=\"" << auxSvcsStatus << "\"" */
	    <<" AuxStatus=\"" << auxStatusEcoGem << "\""
          ...
          of
            <<" LinkId=\"" << link_id << "\""
            <<" NECF=\"" << mean_energy_removed_by_edge / lastLane->getLength() << "\""
            //<< " NumCell=\"" << veh.getMyType()->getNumCells() <<"\""
            //<<" RelHumidity=\"" << getMyType()->getHumidity() << "\""
            <<" RelHumidity=\"" << MSNet::getInstance()->getCurrentEnvHum() << "\""
            //<<" Temperature=\"" << getMyType()->getCelsius() << "\""
            <<" Temperature=\"" << MSNet::getInstance()->getCurrentEnvTemp() << "\""
          ;
          ...
          // Why two hours earlier???
          time_t then = start +
           MSNet::getInstance()->getCurrentTimeStep() /
           (SECS_IN_CURENT_TIME_STEP * 1000) + 7200;
          ...
          of << " TimeStamp=\""<<buffer<<"\"";
          of << " TravelTime=\""<< time2string(time_on_lane) << "\"";
          time_t now;
          time(&now);
          strftime(buffer, 64, "%Y-%m-%dT%H:%M:%S", gmtime(&now));
          of << " UploadTime=\""<<buffer<< "\"";
          of
            <<" VIN=\""<< getID() << "\""
            <<" Weight=\""<< getMyType()->getMass() << "\""
	    
	    // (smendez) March 6, 2015
	    <<" Distance=\""<< lastLane->getLength() << "\""
	    <<" EnergyConsumption=\""<< mean_energy_removed_by_edge << "\""
  ...
  {
    /* no output device specified (do nothing) */
    ;
  }
}
...
void MSVehicle::writeNonFevSyntheticData(MSLane*& lastLane)
{
...
#endif
  {
    ...
    of << " timeStampUTC=\""
       ...
       << " EnvTemp=\"" << MSNet::getInstance()->getCurrentEnvTemp() << "\""
       //<< " EnvHum=\"" << getMyType()->getHumidity() << "\""
       << " EnvHum=\"" << MSNet::getInstance()->getCurrentEnvHum() << "\""
       /*
       << " enter_time_on_lane=\"" << time2string(veh.getMyState().getEnterLane()) <<"\""
       << " time_on_lane =\"" << time2string(time_on_lane) << "\""
       << " enter_pos =\"" << veh.getMyState().getMyPosOnLane() <<"\""
