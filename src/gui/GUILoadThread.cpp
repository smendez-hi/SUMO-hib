...
/// @date    Sept 2002
/// @version $Id: GUILoadThread.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
FXint GUILoadThread::run() {
    if(OptionsCont::getOptions().getAnyVerbosity())
      std::cout<<"FXint GUILoadThread::run()\n"<<std::endl;
    GUINet* net = 0;
    int simStartTime = 0;
    int simEndTime = 0;
    ...
    if (MSGlobals::gUseMesoSim) {
        ...
        net = new GUINet(new GUIVehicleControl(),
                         new GUIEventControl(),
                         new GUIEventControl(), new GUIEventControl());
#ifdef HAVE_MESOSIM
    }
...
void GUILoadThread::load(const std::string& file, bool isNet/*, bool ecoGemRouter*/) {
    if(OptionsCont::getOptions().getAnyVerbosity())
      std::cout<<"void GUILoadThread::load(file="<<file<<", isNet="<<(bool)isNet<<")"<<std::endl;
    myFile = file;
    myLoadNet = isNet;
    start();
}
...
void GUILoadThread::retrieveMessage(const MsgHandler::MsgType type, const std::string& msg) {
