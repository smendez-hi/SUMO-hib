...
/// @date    Sept 2002
/// @version $Id: GUIApplicationWindow.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
GUIApplicationWindow::onCmdOpenRecent(FXObject* sender, FXSelector, void* data) {
    ...
    load(file, sender == &myRecentNets/*, ecoGemRouter*/);
    return 1;
}
...
void GUIApplicationWindow::load
 (const std::string& file, bool isNet, /*bool ecoGemRouter, */bool isReload)
{
  OptionsCont &oc=OptionsCont::getOptions();
    // UXIO FIXME DEBUG VERBOSITY
    if(oc.getAnyVerbosity())
      std::cout<<"GUIApplicationWindow::load(file{"<<file<<"}, isNet{"<<isNet<<"}, isReload{"<<isReload<<"}) launching a thread to load files\n"<<std::endl;
    ...
    if (isReload) {
        // UXIO FIXME DEBUG VERBOSITY
        if(oc.getAnyVerbosity())
          printf("GUIApplicationWindow::load(...) isReload\n");
        myLoadThread->start();
        ...
        // UXIO FIXME DEBUG VERBOSITY
        if(oc.getAnyVerbosity())
          printf("GUIApplicationWindow::load(...) myLoadThread->load(file=%s, isNet=%d)\n",file.c_str(),isNet);
        std::cout<<"GUIApplicationWindow::load(...) myLoadThread->load(file="<<file<<", isNet="<<(bool)isNet<<")"<<std::endl;
        myLoadThread->load(file, isNet/*, ecoGemRouter*/);
        setStatusBarText("Loading '" + file + "'.");
    }
    update();
}

GUISUMOAbstractView* GUIApplicationWindow::openNewView()
{
    if (!myRunThread->simulationAvailable()) {
        myStatusbar->getStatusLine()->setText("No simulation loaded!");
        return 0;
...
void GUIApplicationWindow::loadOnStartup(bool run)
{
    myRunAtBegin = run;
    load("", false/*, ecoGemRouter*/);
}
