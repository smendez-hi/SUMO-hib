...
/// @date    Sept 2003
/// @version $Id: MSInternalLane.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
MSInternalLane::MSInternalLane
 (const std::string& id, SUMOReal maxSpeed, SUMOReal length,
  MSEdge* const edge, unsigned int numericalID, const PositionVector& shape,
  SUMOReal width, const SUMOVehicleClasses& allowed,
  const SUMOVehicleClasses& disallowed, SUMOReal slope)
 :MSLane(id, maxSpeed, length, edge, numericalID, shape, width, allowed,
         disallowed, slope),
  myFoesIndex(-1)
{}
...
MSInternalLane::setParentJunctionInformation
 (MSLogicJunction::InnerState* const foescont, unsigned int foesIdx)
{
  myFoesCont = foescont;
...
bool MSInternalLane::moveCritical(SUMOTime t)
{
