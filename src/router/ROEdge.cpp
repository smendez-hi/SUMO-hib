...
/// @date    Sept 2002
/// @version $Id: ROEdge.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
// method definitions
// ===========================================================================
ROEdge::ROEdge(const std::string& id, RONode* from, RONode* to,
               unsigned int index, bool useBoundariesOnOverride, SUMOReal slope,
               bool interpolate)
    : myID(id), mySpeed(-1),
...
SUMOReal
ROEdge::getEffort(const ROVehicle* const veh, SUMOReal time, bool ecoGemRouter)
const {
    UNUSED_PARAMETER(veh);
    SUMOReal ret = 0;
    if (!getStoredEffort(time, ret)) {
      if(ecoGemRouter)
        return ((SUMOReal)myLength / (SUMOReal)mySpeed) * 
               (mySlope + 1);
      else
        return (SUMOReal)myLength / (SUMOReal)mySpeed;
    }
