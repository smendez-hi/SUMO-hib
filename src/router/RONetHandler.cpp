...
/// @date    Sept 2002
/// @version $Id: RONetHandler.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
RONetHandler::parseEdge(const SUMOSAXAttributes& attrs) {
  OptionsCont &oc=OptionsCont::getOptions();
    ...
    SUMOReal slope = attrs.hasAttribute(SUMO_ATTR_SLOPE) ? attrs.getSUMORealReporting(SUMO_ATTR_SLOPE, myCurrentName.c_str(), ok) : 0.;
    // DEBUG PRINT
    if(slope!=0.){
      if(oc.getAnyVerbosity()>1)
        std::cout<<"EDGE WITH NONZERO SLOPE LOADED!\n";
    }else{
      if(oc.getAnyVerbosity()>1)
        std::cout<<"EDGE WITH ZERO SLOPE LOADED\n";
    }
    if (!ok) {
        return;
    }
    ...
    // build the edge
    myCurrentEdge = myEdgeBuilder.buildEdge(myCurrentName, fromNode, toNode, slope);
