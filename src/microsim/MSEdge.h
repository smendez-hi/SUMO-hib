...
/// @version $Id: MSEdge.h 11671 2012-01-07 20:14:30Z behrisch $
///
...
public:
     ...
     *
     * @param[in] allowed Which edges may be reached from which lanes
     * @param[in] lanes List of this edge's lanes
     * @param[in] function A basic type of the edge
     * @param[in] slope Optional slope for that edge
     */
    ...
    /** @brief Returns this edge's slope
     *
     * @return This edge's slope
     */
    /* const? Why? */ const SUMOReal getSlope() const {
        return mySlope;
    }
    
    /** @brief Set a slope for this edge
     *  @param[in] arg slope to be set
     */
    void setSlope(SUMOReal arg);
    
    /** @brief Get the status of the slope set
     *  @return true if slope has been set, false otherwise
     */
    const bool getSlopeSetStatus()const{return slopeSet;}
     ...
     */
    const std::vector<MSLane*>* allowedLanes
      (const MSEdge& destination, SUMOVehicleClass vclass = SVC_UNKNOWN) const ;
...
public:
    ...
    MSJunction *getFromJunction() /*const */;
    
    void setFromJunction(MSJunction *arg);
    
    MSJunction *getToJunction() /*const */;
    
    void setToJunction(MSJunction *arg);
    
    std::string getNameOfMyToJunction() const;
    
    void setNameOfMyToJunction(std::string arg);
