...
/// @date    Mon, 9 Jul 2001
/// @version $Id: NLJunctionControlBuilder.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
NLJunctionControlBuilder::NLJunctionControlBuilder
 (MSNet& net, NLDetectorBuilder& db)
...
void NLJunctionControlBuilder::openJunction
 (const std::string& id, const std::string& key, const std::string& type,
  SUMOReal x, SUMOReal y, const PositionVector& shape,
  const std::vector<MSLane*> &incomingLanes,
  const std::vector<MSLane*> &internalLanes, const SUMOReal z)
 ...
 () const
{
  ...
  {
    std::cout<<"----> MSJunctionControl *NLJunctionControlBuilder::build() const"<<std::endl;
    std::cout<<" myJunctions->size()="<<myJunctions->size()<<std::endl;
    for(int i=0; i<myJunctions->size(); i++)
    {
      std::cout<<"  myJunctions->getTempVector().at(i)="<<myJunctions->getTempVector().at(i)<<std::endl;
      std::cout<<"  myJunctions->getTempVector().at(i)->getID()="<<myJunctions->getTempVector().at(i)->getID()<<std::endl;
    }
    /*
    for(int i=0; i<myJunctions->size(); i++)
    {
...
MSJunction* NLJunctionControlBuilder::buildNoLogicJunction
 ()
{
  return new MSNoLogicJunction(myActiveID, myPosition, myShape,
    myActiveIncomingLanes, myHeight
#ifdef HAVE_INTERNAL_LANES
...
throw(InvalidArgument)
{
  ...
  // build the junction
  return new MSRightOfWayJunction(myActiveID, myPosition, myShape,
    myActiveIncomingLanes, myHeight,
#ifdef HAVE_INTERNAL_LANES
    myActiveInternalLanes,
#endif
...
{
  // build the junction
  return new MSInternalJunction(myActiveID, myPosition, myShape,
    myActiveIncomingLanes, myHeight, myActiveInternalLanes);
}
#endif
...
void NLJunctionControlBuilder::closeTrafficLightLogic() throw(InvalidArgument, ProcessError) {
    ...
    SUMOTime offset = (int)(myNet.getCurrentTimeStep() + (int)myOffset % (int)myAbsDuration) % (int)myAbsDuration;
#endif
...
void NLJunctionControlBuilder::addLogicItem
 (int request, const std::string& response, const std::string& foes,
...
void NLJunctionControlBuilder::initTrafficLightLogic(const std::string& id, const std::string& programID,
...
void NLJunctionControlBuilder::addPhase(SUMOTime duration, const std::string& state,
