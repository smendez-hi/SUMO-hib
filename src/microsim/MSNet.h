...
/// @version $Id: MSNet.h 11671 2012-01-07 20:14:30Z behrisch $
///
...
public:
    ...
    /**
     * format is 2006-05-04T18:13:52 alike
     */
    int extractYear(std::string dateTimeString);
    int extractMonth(std::string dateTimeString);
    int extractDay(std::string dateTimeString);
    int extractHour(std::string dateTimeString);
    int extractMinutes(std::string dateTimeString);
    int extractSeconds(std::string dateTimeString);
    
    /**
     * depends on month and time
     */
    bool isNight(std::string dateTimeString);
    ...
    /**
     * depends on options (real life (depends on month and hour) or random) and
     * simulating city
     */
    void updateWeather();
...
//    /** make a new struct tm with a 2006-05-04T18:13:52 alike std::string */
//    struct tm timeStringToTm(std::string timeString);
    
    /**   make a new 2006-05-04T18:13:52 alike std::string with the simulation
     *  start and the currentSimStep
     */
    std::string makeTimeString(SUMOTime timeStep);
  ...
  SUMOReal getCurrentEnvHum();
  
  void setCurrentEnvHum(SUMOReal ceh);
  
  SUMOReal getCurrentEnvTemp();
  
  void setCurrentEnvTemp(SUMOReal cet);
 ...
 protected:
    ...
    SUMOReal currentEnvHum;
    
    SUMOReal currentEnvTemp;
