...
/// @author  Sascha Krieg
/// @date    Sept 2002
/// @version $Id: MSTLLogicControl.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
bool MSTLLogicControl::WAUTSwitchProcedure::isPosAtGSP
 (SUMOTime currentTime, const MSTrafficLightLogic& logic){
    //SUMOTime gspTime = TIME2STEPS(getGSPValue(logic)) % logic.getDefaultCycleTime();
    SUMOTime gspTime = (int)TIME2STEPS(getGSPValue(logic)) % (int)logic.getDefaultCycleTime();
...
SUMOTime MSTLLogicControl::WAUTSwitchProcedure::getDiffToStartOfPhase
 (MSTrafficLightLogic& logic, SUMOTime toTime)
{
...
MSTLLogicControl::WAUTSwitchProcedure_Stretch::adaptLogic(SUMOTime step) {
    ...
    if ((deltaPossible > deltaToCut) && (deltaToCut < (cycleTime / 2))) {
        ...
        SUMOTime deltaToStretch = (int)(cycleTime - deltaToCut) % (int)cycleTime;
...
MSTLLogicControl::addWAUTSwitch(const std::string& wautid,
    ...
    s.when = (int)(myWAUTs[wautid]->refTime + when) % (int)86400000;
