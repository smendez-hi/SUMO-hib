...
/// @date    Sept 2002
/// @version $Id: RORDLoader_TripDefs.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
...
RORDLoader_TripDefs::RORDLoader_TripDefs
 (RONet& net, SUMOTime begin, SUMOTime end, bool emptyDestinationsAllowed,
  bool withTaz, const std::string& fileName)
  ...
  myParameter(0), myHaveWarnedAboutDeprecatedTripDef(false)
{
  ...
  if(oc.getLoadVerbosity())
    std::cout<<"----> RORDLoader_TripDefs::RORDLoader_TripDefs(...)"<<std::endl;
}

RORDLoader_TripDefs::~RORDLoader_TripDefs(){}

void RORDLoader_TripDefs::myStartElement
 (int element, const SUMOSAXAttributes& attrs)
{
  OptionsCont &oc = OptionsCont::getOptions();
  if(oc.getLoadVerbosity())
    std::cout<<"----> void RORDLoader_TripDefs::myStartElement(...)"<<std::endl;
    if (element == SUMO_TAG_TRIP__DEPRECATED && !myHaveWarnedAboutDeprecatedTripDef) {
      if(oc.getLoadVerbosity())
        std::cout<<"[100] void RORDLoader_TripDefs::myStartElement(...)"<<std::endl;
    ...
    if (element == SUMO_TAG_TRIP || element == SUMO_TAG_TRIP__DEPRECATED) {
      if(oc.getLoadVerbosity())
        std::cout<<"[200] void RORDLoader_TripDefs::myStartElement(...)"<<std::endl;
        bool ok = true;
    ...
    if (element == SUMO_TAG_VTYPE || element == SUMO_TAG_VTYPE__DEPRECATED) {
      if(oc.getLoadVerbosity())
        std::cout<<"[800] void RORDLoader_TripDefs::myStartElement(...)"<<std::endl;
        myCurrentVehicleType = SUMOVehicleParserHelper::beginVTypeParsing(attrs);
    } else if (myCurrentVehicleType != 0) {
      if(oc.getLoadVerbosity())
        std::cout<<"[900] void RORDLoader_TripDefs::myStartElement(...)"<<std::endl;
