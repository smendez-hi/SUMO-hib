...
/// @version $Id: duarouter_main.cpp 11671 2012-01-07 20:14:30Z behrisch $
///
// Main for DUAROUTER
/****************************************************************************/
...
void initNet(RONet& net, ROLoader& loader, OptionsCont& oc)
{
  ...
  if(oc.getLoadVerbosity())
    std::cout<<"----> void initNet(RONet& net, ROLoader& loader, OptionsCont& oc)"<<std::endl;
    // load the net
    // HI-Iberia original follows
    RODUAEdgeBuilder builder(oc.getBool("weights.expand"), oc.getBool("weights.interpolate"));
    // Try #1
    //CODE//ROAbstractEdgeBuilder builder(oc.getBool("weights.expand"), oc.getBool("weights.interpolate"));
    loader.loadNet(net, builder);
    // load the weights when wished/available
    if (oc.isSet("weight-files")) {
      if(oc.getLoadVerbosity())
        std::cout<<"[100] void initNet(RONet& net, ROLoader& loader, OptionsCont& oc)"<<std::endl;
        loader.loadWeights(net, "weight-files", oc.getString("weight-attribute"), false);
    }
    if (oc.isSet("lane-weight-files")) {
      if(oc.getLoadVerbosity())
        std::cout<<"[200] void initNet(RONet& net, ROLoader& loader, OptionsCont& oc)"<<std::endl;
...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    if(oc.getLoadVerbosity())
    {
      if(net->getVehIDs().size())
      {
      std::cout<<"?"<<std::endl;
      std::cout<<"net->getVehIDs().size()="<<net->getVehIDs().size()<<std::endl;
      std::vector<std::string> vehIds =
        std::vector<std::string>(net->getVehIDs().begin(),net->getVehIDs().end());
      std::cout<<"net->getVehIDs()=";
      int i;
      for(i=0;i<vehIds.size()-1;i++)
      {
        std::cout<<vehIds.at(i);
        std::cout<<",";
      }
      std::cout<<vehIds.at(i);
      std::cout<<std::endl;
    ...
    // Print cars again
    if(oc.getLoadVerbosity()&&false/*because builder from iterators keeps failing*/)
    {
      if(net->getVehIDs().size())
      {
      std::cout<<"?"<<std::endl;
