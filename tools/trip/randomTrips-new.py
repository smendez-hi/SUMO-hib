...
@date    2010-03-06
@version $Id: randomTrips.py 16365 2014-05-13 08:50:42Z namdre $
...
def get_options():
    ...
    optParser.add_option("-e", "--end", type="int", default=3600, help="end time (default 3600)")
    optParser.add_option("-p", "--period", type="int", default=1, help="repetition period (default 1)")
    ...
    optParser.add_option('-S','--start',dest='startTime',default='0',
                         help='start hour for the generation of traffic, goes from 0 (default) to 23 (max)')
    optParser.add_option('-D', '--specify-day', dest = 'day', type = 'int',
                         default = 0, help = 'Specify a day from 0 for monday to 6 for sunday (default monday)')
...
<trips>""" % (datetime.datetime.now(), (' '.join(sys.argv[1:]).replace('--','<doubleminus>')))
        ...
        #         else:
        #             print >> fouttrips, '    <trip id="%s" depart="%.2f" from="%s" to="%s" %s%s/>' % (
        #                     label, depart, '56710833', '23063990', via, options.tripattrs)
        #     except Exception, exc:
        #         print exc
        #     idx += 1
